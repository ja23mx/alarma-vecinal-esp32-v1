...
void NeoPixelController::ejecutarPatronServidorConectado(void)
{
    ...
    if (reset)
    {
        ...
        // [PASO 1 - LIMPIEZA] Variables anteriores comentadas - nuevo sistema de turnos
        // inicioPeriodoOFF = millis(); // Registrar inicio del período OFF
        // blinkMP3EjecutadoEnEsteCiclo = false; // Reset flag del MP3
        Serial.println("[PASO 1] Patrón servidor conectado - iniciado");
    }
    ...
    if (cambioEstado)
    {
        ...
        // [PASO 1 - LIMPIEZA] Reset del período OFF comentado - nuevo sistema de turnos
        // TODO: Implementar contador de ciclos en Paso 2
        /*
        if (!estadoLED) {
...
void NeoPixelController::ejecutarPatronServidorDesconectado(void)
{
    ...
    if (reset)
    {
        ...
        // inicioPeriodoOFF = millis(); // Registrar inicio del período OFF
        // blinkMP3EjecutadoEnEsteCiclo = false; // Reset flag del MP3
        Serial.println("[PASO 1] Patrón servidor desconectado - iniciado");
    }
